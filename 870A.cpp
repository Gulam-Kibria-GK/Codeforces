...
int main()
{
    ...
    vector<int>v1;
    vector<int>v2;
    for(i=0; i<n; i++)sf1(k),v1.push_back(k);
    for(i=0; i<m; i++)sf1(k),v2.push_back(k);
    sort(v1.begin(),v1.end());
    sort(v2.begin(),v2.end());
//    for(i=0;i<v1.size();i++){if(v1[i]==v2[0]){f=1;break;}}
//    if(f!=0){for(i=0;i<v2.size();i++){if(v2[i]==v1[0]){f=1;break;}}}
    for(i=0;i<v1.size();i++){
        for(j=0;j<v2.size();j++){
            if(v1[i]==v2[j]){
                f=i;
    ...
    if(f!=-2){cout<<v1[f]<<endl;return 0;}
    j=min(v1[0],v2[0]);
    l=max(v1[0],v2[0]);
    cout<<(10*j)+l<<endl;
    return 0;
