...
int main()
{
    int i,j,k,l,a,n,m,b=0;
    bool f1,f2,f3,f4;
    f1=f2=f3=f4=1;
    sf1(a);
    vector<int>v;
    int vv[a+7];
    for(i=0;i<a;i++)sf1(vv[i]);
    sort(vv,vv+a);
    for(i=0;i<a;i++){
        b=vv[i];
        if(b==100){
            if(f3)f3=0,v.push_back(b);
        }
        else if(b>=10){
            if(f2 && b%10==0)f2=0,f1=0,v.push_back(b);
            else if(f1 && b%10)f1=0,f2=0,v.push_back(b);
        }
        else if(b>=1){
            if(f1)f1=0,v.push_back(b);
        }
        else {if(f4)f4=0,v.push_back(b);}
    }
    cout<<v.size()<<endl;
    for(i=0;i<v.size();i++){
        cout<<v[i]<<" ";
