...
int main()
{
    ll i,j=0,k=0,l,a,b[5],n;
    vector<int>m;
    vector<int>m1;
    vector<pair<int,pair<int,int> > >v;
    for(i=0; i<3; i++)sfl1(a),m.push_back(a);
    for(i=0; i<3; i++)sfl1(a),m1.push_back(a);
    for(i=0; i<3; i++)v.push_back({m[i]-m1[i],{m[i],m1[i]}});
    sort(v.rbegin(),v.rend());
    //for(i=0;i<3;i++)cout<<v[i].first<<" "<<v[i].second<<endl;
    for(i=0; i<3; i++)
    {
        if((v[i].second.first-v[i].second.second)>=0)
        {
            k++;
            b[j++]=(v[i].second.first-v[i].second.second)/2;
        }
    }
    ...
    {
        ll c;
        c=(b[0]+b[1]+v[2].second.first);
        if(v[2].second.second<=c)cout<<"Yes"<<endl;
        else cout<<"No"<<endl;
    }
    else
    {
        if(v[1].second.second<=(b[0]+v[1].second.first))
        {
            b[0]=(b[0]+v[1].second.first)-(v[1].second.second);
            if(v[2].second.second<=(b[0]+v[2].second.first))
            {
                cout<<"Yes"<<endl;
            }
            else cout<<"No"<<endl;
